...
| Cond(e, c1, c2) -> (separate space [string "if"; doc_of_expr e; colon]) ^^
...
| Fundefn(FPdecl(rt, fn, params), e)->
      (separate space [string "def"; string fn; (doc_of_var_list (List.map name_of_vardecl params)); colon]) ^^
      (nest indent_level (hardline ^^ (string "return"  ^^ parens (doc_of_expr e))))
...
(string "print" ^^ (parens (string "str" ^^ (parens (doc_of_expr e))))) ^^ hardline
